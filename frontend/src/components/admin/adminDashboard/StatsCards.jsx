      ...
      <div className="grid gap-4 mb-6" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))' }}>
        {[1, 2, 3, 4, 5, 6].map(i => (
          <div
            ...
            style={{ background: cardBg, borderColor: borderCol, boxShadow: cardShadow }}
          >
    ...
    <div className="grid gap-4 mb-6" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))' }}>
      {stats.map(({ label, value, delta, icon: Icon, iconBg, iconColor }) => {
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: iconBg || '#615FFF' }}>
                <SafeIcon size={22} style={{ color: iconColor || '#fff' }} />
              </div>
