...
const getResultBg = (valid, dark) => {
  if (valid) {
    return dark ? 'rgba(0,188,125,0.1)' : '#e6fbf2'
  }
  return dark ? 'rgba(239,68,68,0.1)' : '#fef2f2'
}
...
export default function CertVerifyModal({
  ...
  useEffect(() => {
    if (verifyOpen && dialogRef.current && !dialogRef.current.open) {
  ...
  return createPortal(
    ...
    >
      ...
      >
        <div
          className="pointer-events-auto rounded-[24px] w-full max-w-[440px] overflow-hidden"
          style={{
        ...
        >
          ...
          <div className="px-7 py-6 space-y-4">
            <div>
              <label htmlFor="verifyCodeInput" className="text-[13px] font-bold block mb-2" style={{ color: tokens.txtSec }}>Enter Verify Code</label>
              <input
                id="verifyCodeInput"
                type="text"
            ...
            {verifyResult && (
              <div
                ...
                style={{
                  background: getResultBg(verifyResult.valid, dark),
