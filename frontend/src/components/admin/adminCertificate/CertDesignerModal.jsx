...
function getOutlineStyle(borderStyle, accentColor) {
  if (borderStyle === 'none') return 'none'
  if (borderStyle === 'thin') return `1px solid ${accentColor}`
  if (borderStyle === 'thick') return `3px solid ${accentColor}`
  return `4px double ${accentColor}`
}

function getToggleBg(active, dark, BRAND) {
  if (active) return BRAND
  return dark ? '#1a3050' : '#cbd5e1'
}
...
export default function CertDesignerModal({
  ...
  useEffect(() => {
    if (designerOpen && dialogRef.current && !dialogRef.current.open) {
  ...
  return createPortal(
    <dialog
      ref={dialogRef}
      className="fixed inset-0 z-110 m-0 p-0 w-full h-full border-none bg-transparent"
        ...
        >
          ...
          >
            ...
            <div className="flex-1 p-5 space-y-5">
              ...
              {/* Organisation Name */}
              <div>
                <label htmlFor="certOrgInput" className="text-[11px] font-bold uppercase tracking-wider block mb-2" style={{ color: tokens.txtSec }}>Organisation Name</label>
                <input
                  id="certOrgInput"
                  value={tmpl.org}
              ...
              {/* Certificate Title */}
              <div>
                <label htmlFor="certTitleInput" className="text-[11px] font-bold uppercase tracking-wider block mb-2" style={{ color: tokens.txtSec }}>Certificate Title</label>
                <input
                  id="certTitleInput"
                  value={tmpl.title}
              ...
              {/* Gradient Colors */}
              <div>
                <label htmlFor="certGradFrom" className="text-[11px] font-bold uppercase tracking-wider block mb-2" style={{ color: tokens.txtSec }}>Background Gradient</label>
                <div className="flex gap-2">
                  {[['gradFrom', 'From', 'certGradFrom'], ['gradMid', 'Mid', 'certGradMid'], ['gradTo', 'To', 'certGradTo']].map(([key, lbl, inputId]) => (
              ...
              {/* Accent Color */}
              <div>
                <label htmlFor="certAccentColor" className="text-[11px] font-bold uppercase tracking-wider block mb-2" style={{ color: tokens.txtSec }}>Accent / Logo Color</label>
              ...
              {/* Border Style */}
              <div>
                <span className="text-[11px] font-bold uppercase tracking-wider block mb-2" style={{ color: tokens.txtSec }}>Border Style</span>
                <div className="flex gap-2">
              ...
              {/* Font Family */}
              <div>
                <label htmlFor="certFontFamily" className="text-[11px] font-bold uppercase tracking-wider block mb-2" style={{ color: tokens.txtSec }}>Font Family</label>
              ...
              <div className="space-y-3">
                <span className="text-[11px] font-bold uppercase tracking-wider block" style={{ color: tokens.txtSec }}>Options</span>
                {[['showLogo', 'Show Logo'], ['showSignatures', 'Show Signatures']].map(([key, lbl]) => (
                  <div key={key} className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      style={{ background: getToggleBg(tmpl[key], dark, BRAND) }}
                    >
                      <div
            ...
            <div className="p-5" style={{ borderTop: `1px solid ${tokens.border}` }}>
              <button
                ...
                style={{ background: tmpl.templateSaved ? '#00BC7D' : BRAND, boxShadow: tmpl.templateSaved ? '0 4px 14px rgba(0,188,125,0.4)' : `0 4px 14px ${BRAND}40` }}
              >
            ...
            <div className="flex-1 flex items-center justify-center p-8 overflow-auto" style={{ background: dark ? '#060e1c' : '#f1f5f9' }}>
              ...
              <div
                ...
                style={{
                  ...
                  outline: getOutlineStyle(tmpl.borderStyle, tmpl.accentColor),
                  outlineOffset: '6px',
                }}
              >
                {/* Blobs */}
                ...
                <div className="relative z-10 flex flex-col items-center text-center gap-2.5">
                  ...
                  <p className="text-[18px] font-extrabold m-0" style={{ color: tmpl.accentColor ? `${tmpl.accentColor}cc` : '#a5b4fc' }}>Event Name</p>
