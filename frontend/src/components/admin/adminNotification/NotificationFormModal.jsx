...
function SendButtonContent({ sending, sent }) {
  if (sent) return <><CheckCheck size={15} /> Sent!</>
  if (sending) return <>Sending...</>
  return <><Send size={14} /> Send Now</>
}
...
export default function NotificationFormModal({
  ...
  useEffect(() => {
    if (sendOpen && dialogRef.current && !dialogRef.current.open) {
        ...
        >
          ...
          <div className="px-6 py-5 flex flex-col gap-5 max-h-[70vh] overflow-y-auto">
            <NotificationFormFields
              ...
              handleCustomUserIdChange={handleCustomUserIdChange}
              userRole={userRole}
              dark={dark}
              BRAND={BRAND}
          ...
          <div className="flex gap-3 px-6 py-4" style={{ borderTop: `1px solid ${dark ? '#1a3050' : '#e8edf5'}` }}>
            ...
            >
              <SendButtonContent sending={sending} sent={sent} />
            </button>
          </div>
        </div>
      </div>
