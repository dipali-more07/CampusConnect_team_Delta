...
export default function CertificatesPage({ tokens }) {
  ...
  const handleVerify = async () => {
    ...
    if (res.success && res.certificate) {
      ...
      const localCert = certs.find(c => c.verifyCode === code || c.certificate_number === code)
      
      let enriched = { ...res.certificate }
      if (localCert) {
        enriched = { ...enriched, ...localCert }
      }
      
      if (enriched.generated_at && !enriched.issuedDate) {
        try {
          enriched.issuedDate = new Date(enriched.generated_at).toISOString().split('T')[0]
        } catch (e) {
          enriched.issuedDate = enriched.generated_at
        }
      }
      
      if (!enriched.eventName && enriched.event_id) {
        const ev = allEvents.find(e => String(e.id || e.event_id) === String(enriched.event_id))
        if (ev) enriched.eventName = ev.name
      }
      
      if (!enriched.studentName && enriched.user_id) {
        const stuRes = await studentsService.fetchAll()
        if (stuRes.success) {
          const st = (stuRes.students || []).find(s => String(s.id) === String(enriched.user_id))
          if (st) {
            enriched.studentName = st.name || st.full_name
            enriched.rollNo = st.rollNo || st.roll_no || st.college_id
          }
        }
      }
      
      res.certificate = enriched
    }
