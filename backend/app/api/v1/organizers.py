...
def assign_organizer(
    ...
    return success_response(message="Organizer assigned", data={"organizer_id": organizer.organizer_id, "club_id": organizer.club_id, "designation": organizer.designation}, status_code=201)
...
def list_organizers(
    ...
    data = [{"organizer_id": o.organizer_id, "user_id": o.user_id, "club_id": o.club_id, "designation": o.designation, "permissions": o.permissions} for o in organizers]
...
def get_my_organizer_profile(
    ...
    return success_response(message="Organizer profile fetched", data={"organizer_id": org.organizer_id, "club_id": org.club_id, "designation": org.designation, "permissions": org.permissions})


@router.get("/{organizer_id}", summary="Get organizer by ID")
def get_organizer(organizer_id: str, db: Session = Depends(get_db)):
    service = OrganizerService(db)
    org = service.get_organizer(organizer_id)
    return success_response(message="Organizer fetched", data={"organizer_id": org.organizer_id, "user_id": org.user_id, "club_id": org.club_id})
